        ...
        class Woo_Variation_Swatches_Product_Page {
            ...
            public function color_attribute( $data, $attribute_type, $variation_data = array() ) {
                ...
                if ( 'color' === $attribute_type ) {
                    ...
                    // Get product ID and variation name
                    $product_id = $product->get_id();
                    $variation_name = isset( $term->name ) ? $term->name : '';
                    
                    // Get the complete variation name (e.g., '32-navy') from the 'option_slug' or 'slug'
                    $variation_value = isset( $data['option_slug'] ) ? $data['option_slug'] : (isset( $data['slug'] ) ? $data['slug'] : '');
            
                    // Add product ID, variation name, and variation value to the data attributes
                    $template_format = apply_filters( 'woo_variation_swatches_color_attribute_template', '<span class="variable-item-span variable-item-span-color variant-image" style="background-color:%s;" data-product_id="%s" data-variation_name="%s" data-value="%s"></span>', $data, $attribute_type, $variation_data );
                    
                    return sprintf( $template_format, esc_attr( $color ), esc_attr( $product_id ), esc_attr( $variation_name ), esc_attr( $variation_value ) );
                }
            }
            ...
            //         // Add product ID and variation name to the data attributes
            //         $template_format = apply_filters( 'woo_variation_swatches_color_attribute_template', '<span class="variable-item-span variable-item-span-color variant-image" style="background-color:%s;" data-product_id="%s" data-variation_name="%s"></span>', $data, $attribute_type, $variation_data );
                    
            //         return sprintf( $template_format, esc_attr( $color ), esc_attr( $product_id ), esc_attr( $variation_name ) );
            //     }
            // }
